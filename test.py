...
def main():
    ...
    print('===> Start Test')
    print("==================================================")
    print("Method: %s || Quality: %d "%(model_name, quality ))
    ...
    for bm, test_loader in zip(bm_names, test_loaders):
        ...
        if need_HR:
            save_img_path = os.path.join('./results/SR/' ,model_name, bm, "x%d"%quality)
        else:
            save_img_path = os.path.join('./results/SR/'+bm, model_name, "x%d"%quality)
